defmodule Day3 do
  ...
  """
  def solve() do
    data = Loader.load_data("day_3_input.txt")

    gamma_rate_bits = data |> calculate_most_common_bits()
  ...
  def calculate_most_common_bits(data) do
    number_length =
      data
      |> Enum.at(0)
      |> String.length()
    ...
    data
    |> process()
    |> Enum.reduce(List.duplicate(0, number_length), &sum_bit_values/2)
    |> IO.inspect()
    |> Enum.map(fn x ->
      if x >= total_number_of_values / 2 do
        1
      else
        0
      end
  ...
  def calculate_least_common_bits(data) do
    number_length =
      data
      |> Enum.at(0)
      |> String.length()

    total_number_of_values = data |> length()
  ...
  def sum_bit_values(current, accum) do
    current
    |> Enum.zip_with(accum, &(&1 + &2))
  end

  def convert_to_integer(list_of_bits) do
    list_of_bits
    |> Enum.reverse()
    |> Enum.reduce({0, 0}, fn x, {sum, exponent} -> {sum + x * 2 ** exponent, exponent + 1} end)
    |> then(fn {sum, _} -> sum end)
  end

  def invert_bits(list_of_bits) do
    list_of_bits
    |> Enum.map(fn x ->
      case x do
        0 -> 1
...
defmodule Day3Part2 do
  ...
  Start with all 12 numbers and consider 6only the first bit of each number. There are more 1 bits (7) than 0 bits (5), so keep only the 7 numbers with a 1 in the first position: 11110, 10110, 10111, 10101, 11100, 10000, and 11001.
  ...
  def solve() do
    ...
    oxygen_generator_rating =
      ...
      |> IO.inspect(label: "oxygen generator rating")

    carbon_dioxide_scrubber_rating =
      data
      |> calculate_carbon_dioxide_scrubber_rating()
      |> IO.inspect(label: "CO2 scrubber rating")

    _life_support_index =
      (carbon_dioxide_scrubber_rating * oxygen_generator_rating)
